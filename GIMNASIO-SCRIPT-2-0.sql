...
PRIMARY KEY (ID_PAGO),
CONSTRAINT FK_PAGO FOREIGN KEY (IDUSUARIO) REFERENCES USUARIO(IDUSUARIO)
);
...
----------------------------------CREAMOS TABLA HISTORICA DE INVENTARIO--------------------------------------------------------
...
FOR EACH ROW 
BEGIN 
    INSERT INTO INVENTARIO_HISTORICO VALUES(:NEW.ID_EQUIPO, :NEW.NOMBRE, :NEW.CANTIDAD, :NEW.FECHA, NULL );	
...
create or replace FUNCTION SP_PAGOS (FECHA_DIA IN DATE)
return number 
AS
    v_pagosDia number(8);

BEGIN
     SELECT sum(monto) INTO v_pagosDia 
     FROM PAGO WHERE FECHA=FECHA_DIA;
...
create or replace FUNCTION SP_TELEFONOS (TIPO NUMBER)
return VARCHAR2 
AS
    v_TIPO_TEL VARCHAR2(50);

BEGIN
    v_tipo_tel:='';
    IF TIPO=1 THEN
        v_TIPO_TEL:='TELEFONO PERSONAL';
    ELSIF TIPO=2 THEN
        v_TIPO_TEL:='TELEFONO DE CASA';
    ELSIF TIPO=3 THEN
        v_TIPO_TEL:='TELEFONO DE TRABAJO';
    ELSE
        v_TIPO_TEL:='TIPO NO ENCONTRADO';
    END IF;
    RETURN v_tipo_tel;
END;
...
create or replace procedure ingresoIMC(FECHA varchar2,IMC_USUARIO VARCHAR2,IDUSUARIO NUMBER)
AS
    BEGIN 
        INSERT INTO IMC VALUES(SEC_IMC.NEXTVAL,FECHA,IMC_USUARIO,IDUSUARIO);
END ingresoIMC;




create or replace procedure ingresoKcal(CALORIAS_DIA NUMBER,FECHA VARCHAR2, IDUSUARIO NUMBER)
AS
    BEGIN 
        INSERT INTO CALORIAS_DIARIAS VALUES(SEC_Kcal.NEXTVAL,CALORIAS_DIA,FECHA,IDUSUARIO);
END ingresoKcal;
...
IS
    FUNCTION SP_PAGOS (FECHA_DIA IN DATE)return number ;
    FUNCTION SP_TELEFONOS (TIPO NUMBER)return VARCHAR2 ;
END;

CREATE OR REPLACE PACKAGE BODY FUNCTIONS
IS
    FUNCTION SP_PAGOS (FECHA_DIA IN DATE)
    return number 
    AS
        v_pagosDia number(8);
    ...
    FUNCTION SP_TELEFONOS (TIPO NUMBER)
    return VARCHAR2 
    AS
        v_TIPO_TEL VARCHAR2(50);
...
CREATE OR REPLACE PACKAGE PROCEDURES
IS
    procedure ingresoIMC(FECHA varchar2,IMC_USUARIO VARCHAR2,IDUSUARIO NUMBER);
    procedure ingresoKcal(CALORIAS_DIA NUMBER,FECHA VARCHAR2, IDUSUARIO NUMBER);
END;

CREATE OR REPLACE PACKAGE BODY PROCEDURES
IS
    procedure ingresoIMC(FECHA varchar2,IMC_USUARIO VARCHAR2,IDUSUARIO NUMBER)
    AS
    BEGIN 
